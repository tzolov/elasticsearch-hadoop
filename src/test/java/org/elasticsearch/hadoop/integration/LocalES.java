...
public class LocalES extends ExternalResource {
    ...
    public void refresIndex(String indexName) {
      new RefreshRequestBuilder(master.getClient().admin().indices()).setIndices(indexName).execute().actionGet();
    }

    public long countIndex(String indexName, String typeName) {
      return master.getClient().prepareCount(indexName).setTypes(typeName).execute().actionGet().getCount();
    }

    public Iterator<SearchHit> searchIndex(String indexName, String typeName) {
      return master.getClient().prepareSearch(indexName).setTypes(typeName).execute().actionGet().getHits().iterator();
    }
