...
public class CrunchAvroIT implements Serializable {
  ...
  public void testWriteAvroToES() throws InterruptedException, IOException {
    ...
    pipeline.write(personCollection, new ESTarget("person/avro"));
...
//    readPersonFromES();
  ...
  private void readAsTextFromESIndex() {
    ...
    PCollection<String> rawCollection = pipeline.read(
        new ESTypedSource<String>("person/avro/_search?q=*", String.class)).parallelDo(identityFn, tf.strings());
  ...
  public void readPersonFromES() {
    ...
    PCollection<Person> people = pipeline.read(new ESTypedSource<Person>("person/avro/_search?q=*", Person.class)).parallelDo(identityFn,
        tf.records(Person.class));
